 ...
 * payload in any of those fields could otherwise smuggle HTML into the
 * GitHub-rendered Markdown surface (F-T1 / F-T3 from the 2026-04-19 review).
 *
 * Strategy:
 ...
 *   - escape ``` ` ``` runs of >= 3 backticks so the digest can't break out
 *     of any wrapping fenced-code block we add later
 */
function sanitizeDigestForMarkdown(body: string): string {
  ...
  const escaped = cappedLines
    ...
    .map((line) => line.replace(/`{3,}/g, (m) => '`'.repeat(m.length).replace(/`/g, '\\`')));
  return escaped.join('\n');
}
